...
import dev.ykh.triplehomework.web.response.v1.PointsSubView
import dev.ykh.triplehomework.web.response.v1.PointsView
...
class PointsService(
    ...
    suspend fun add(userId: String, pointsRequest: PointsRequest) {
        ...
        if(pointsRequest.content.isNotEmpty() &&
            ...
            points.add(Points(userId, addContentsPoints, pointsRequest.type, EventDetailType.ADD_CONTENTS, pointsRequest.placeId, pointsRequest.reviewId))
        }
        ...
        if(pointsRequest.attachedPhotoIds.isNotEmpty() &&
            ...
            points.add(Points(userId, addPhototsPoints, pointsRequest.type, EventDetailType.ADD_PHOTOS, pointsRequest.placeId, pointsRequest.reviewId))
        }
        ...
        //더불어 이전에 해당 이유로 본인이 포인트를 받은 이력이 있는지 확인
        if(placesReviewsRepository.findByPlaceIdAndDeletedOrderByCreatedAtAsc(pointsRequest.placeId, false)
            ...
            points.add(Points(userId, addFirstReviewPoints, pointsRequest.type, EventDetailType.ADD_FIRST_REVIEW, pointsRequest.placeId, pointsRequest.reviewId))
        }
    ...
    suspend fun mod(userId: String, pointsRequest: PointsRequest) {
        ...
        if(pointsRequest.attachedPhotoIds.isEmpty()) {
            ...
            if(reviewsPhototsRepository.countByReviewIdAndDeleted(pointsRequest.reviewId, false) == zero &&
            ...
            if(pointsRepository.countByUserIdAndPlaceIdAndDetailType(userId, pointsRequest.placeId, EventDetailType.ADD_PHOTOS) == zero &&
                reviewsPhototsRepository.countByReviewIdAndDeleted(pointsRequest.reviewId, false) == zero) {
    ...
    suspend fun delete(userId: String, pointsRequest: PointsRequest) {
        ...
        if(pointsRepository.countByUserIdAndPlaceIdAndDetailType(userId, pointsRequest.placeId, EventDetailType.REMOVE_REVIEW) == zero) {
            ...
            pointsRepository.saveAll(pointsEntities)
        }
    }
}